//! Package-internal scanner implementation and metering tests.
//! The public run-to-completion surface is re-exported by `lexer.zig`;
//! the parser imports this module directly for optional bounded scanning.
//!
    ...
    return struct {
        ...
        fn drive(self: *Self, comptime bounded: bool, budget: usize) Advance {
            ...
            scan_done: {
                ...
                // resets to trivia. Keep its hot entry statically known; only
                // metered scanners need to resume a saved lexical state.
                scan: switch (if (metered) self.state else State.trivia) {
                    inline else => |state| {
                        while (true) {
                            if (bounded) {
                                if (remaining == 0) {
